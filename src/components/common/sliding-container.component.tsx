...
class Container extends Component<Props, State> {
    ...
    render() {
        ...
        return (
            <Animated.View style={[styles.subView, {transform: [{translateY: this.state.springAnim}]}]}>
                <FullScreenMessage
                    message={'Event Created!'}
                    onPress={() => this.theOnPress()}
                ></FullScreenMessage>
    ...
    toggleSubview() {
        let toValue = 0;
        let isHidden = this.state.loading || this.state.message || this.state.error || this.state.success;
        if (!isHidden) {
            toValue = 500;
        }
